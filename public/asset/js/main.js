...
$('#id_finder').on('click', function(){
    ...
    $.ajax({
        ...
        success: function (result) {
            ...
            var contracts = JSON.parse(result).contract;
            var products = JSON.parse(result).product;
            if (datas) {
                $.each(datas, function (index, ele) {
                    $('#id_banner').val(ele.id_banner)
                    $('#_detail_name_banner').val(ele._name_banner)
                    $('#banner_adress').val(ele.banner_adress)
                    $('#province').val(ele.name_province)
                    $('#district').val(ele.name_dt)
                    $('#type_product').val(ele.name_type)
                    $('#id_system').val(ele.id_system)
                    $('#status').val(ele.name_st)
                    $('#size_banner').val(ele.size_banner)
                    $('#dien_tich').val(ele.dien_tich)
                    $('#height_banner').val(ele.height_banner)
                    $('#light_system').val(ele.light_system)
                    $('#flow').val(ele.flow)
                    $('#v_light').val(ele.v_light)
                    $('#escom').val(ele.escom)
                    $('#gianam').val(ele.gianam)
                    $('#content').val(ele.content)
                    $('textarea#dac_diem').text(ele.dac_diem)
                    $('#license_number_advertise').val(ele.license_number_advertise)
                    $('#start_date_advertise').val(ele.start_date_advertise)
                    $('#date_range_advertise').val(ele.date_range_advertise)
                    $('#end_date_advertise').val(ele.end_date_advertise)
                    $('#content_advertise').val(ele.content_advertise)
                    $('#license_number_build').val(ele.license_number_build)
                    $('#start_date_build').val(ele.start_date_build)
                    $('#date_range_build').val(ele.date_range_build)
                    $('#end_date_build').val(ele.end_date_build)
                    $('#content_build').val(ele.content_build)
                    $('#note_banner').val(ele.note_banner)
                    $('#id_banner').val(ele.id_banner)
                })   
                if(contracts){
                    $.each(contracts, function (index, element) {
                        $('#detail_finder_contract').append(
                            `
                            <tr class="status--process dropdown">
                                            <td><input type="checkbox" id="check-box" data-target="" name="check_box[]" onchange="getCheckedBox()" value=""
                                                    class="display-input m-r-5"></td>
                                            <td><a class="text-dark" href="">${element.id_contract}</a></td>
                                            <td><a class="text-dark" href="">${element.name_customer}</a></td>
                                            <td><a class="text-dark" href="">${element.name_kind}</a></td>
                                            <td><a class="text-dark" id="pd" href=""></a></td>
                                            <td><a class="text-dark" href="">${element.date_end}</a></td>
                                            <td class="value_contract"><a class="text-dark" href=""><span>${format(element.vl_contract_vat_vnd)}</span></a></td>
                                            <td><a class="text-dark" href="">0</a></td>
                                            <td><a class="text-dark" href="">0</a></td>
                                            <td><a class="text-dark" href="">${element.name_status}</a></td>
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-6">
                                <div class="row">
                                    ...
                                    <div class="col-md-6 col-sm-12">
                                        <input type="text" class="form-control" value="${ele.size_banner}" id="size_banner_1" name="size_banner[]"
                                               placeholder="Kích Thước" ${ele.readonly}>
                                    </div>
                                </div>
                            </div>
                        </div>
